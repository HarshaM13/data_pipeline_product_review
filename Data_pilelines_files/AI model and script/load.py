...
def load_data():
    ...
    if not git_cmd:
        git_cmd = r"C:\\Program Files\\Git\\bin\\git.exe"  # Fallback path
    ...
    if not os.path.exists(".git"):
        ...
        subprocess.run([git_cmd, "remote", "add", "origin",
                        "https://github.com/jessicabpa9-hub/data_pipeline_product_review.git"], check=True)

    # Stage the processed file
    subprocess.run([git_cmd, "add", file_path], check=True)

    # Configure Git user details
    subprocess.run([git_cmd, "config", "user.name", "jessicabpa9-hub"], check=True)
    subprocess.run([git_cmd, "config", "user.email", "your-email@example.com"], check=True)
    ...
    print(f"Published {file_path} to GitHub at https://github.com/jessicabpa9-hub/data_pipeline_product_review.git")
